	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		total.clear();
...

			{
				try {
					con = DBUtil.getDataSource().getConnection();
					String sql1 = "select DISTINCT datetime from practicalattendance where branch=? and division=? and sem=? and subject=? and rollno=? and month between ? and ? and accyear=?";
					ps = con.prepareStatement(sql1);
					ps.setString(1, branch);
					ps.setString(2, division);
					ps.setString(3, sem);
					...
					// total lecture
					String sql2 = "select DISTINCT datetime from practicalattendance where branch=? and division=? and sem=? and subject=? and  month between ? and ? and accyear=?";
					ps = con.prepareStatement(sql2);
					ps.setString(1, branch);
					ps.setString(2, division);
					ps.setString(3, sem);
